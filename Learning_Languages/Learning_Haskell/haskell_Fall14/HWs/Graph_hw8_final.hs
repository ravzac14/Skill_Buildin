...
showGraph :: Ord a => Graph a -> ([a],[(a,a)])
showGraph (Graph a)		= (toList (fst a), toList (snd a))


vertices :: Ord a => Graph a -> [a]
vertices (Graph a) 		= toList (fst a)

edges :: Ord a => Graph a -> [(a,a)]
edges (Graph a)			= toList (snd a)

adjacent :: Ord a => Graph a -> a -> [a]
adjacent (Graph a) n
	| not (n `elem` (vertices (Graph a)))	= error "Not in graph"
	| otherwise								= iterate' n (edges (Graph a))
		where
			iterate' i []	= []
			iterate' i ((x,y):xys)		
				| i == x	= y:(iterate' i xys)
				| i == y	= x:(iterate' i xys) 
				| otherwise	= iterate' i xys 

isAdjacent :: Ord a => Graph a -> a -> a -> Bool
isAdjacent (Graph a) x y
	| not (x `elem` (vertices (Graph a))) ||
		not (y `elem` (vertices (Graph a)))		= False
	| otherwise									= chimi x y (edges (Graph a))
		where
			chimi a b []			= False
			chimi a b ((n,m):nms)
				| a == n && b == m	= True
				| a == m && b == n	= True
				| otherwise			= chimi a b nms
